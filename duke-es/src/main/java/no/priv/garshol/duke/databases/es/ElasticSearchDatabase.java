...
public class ElasticSearchDatabase implements Database {
    ...
    private static final String[] DATA_SUBDIRS = {"data", "work", "logs", "home"};
    ...
    private int editDistance; // used for fuzzyness calculation
    ...
    public void init() {
        ...
        this.idProperty = new ArrayList<Property>(identityProperties).get(0);
    ...
    private void setupConnection() {
        ...
        if (this.tAddresses == null) {
            ...
            //System.out.println("Creating Transport client: " + settings.toString());
        ...
        System.out.println("ElasticSearch Health Check " + actionGet.toString());
    }
        ...
        for (String propname : record.getProperties()) {
            ...
            if (prop.isIdProperty()) {
                ...
                if (values != null && !values.isEmpty()) {
                    if (values.size() == 1) {
                        json.put(propname, new ArrayList<>(values).get(0));
                    } else {
    ...
    public void commit() {
        if (this.client != null) {
            ...
            this.client.admin().indices().prepareForceMerge(this.indexName)
    ...
    public Collection<Record> findCandidateMatches(Record record) {
        ...
        for (Property prop : config.getLookupProperties()) {
            ...
            if (queryString.length() <= 0)
                continue;
            QueryBuilder qsqb = createQuery(propName, queryString, boostFactor);
            bqb = required ? bqb.must(qsqb) : bqb.should(qsqb);

        }

        if(this.logESQueries) {
            logger.warning(bqb.toString());
        }
    ...
    private QueryBuilder createQuery(String propName, StringBuilder queryString,
                                                Float boostFactor) {
        final String trimmedStr = queryString.toString().trim();
        return this.editDistance > 0 ?
                QueryBuilders.fuzzyQuery(propName, trimmedStr)
                        .fuzziness(Fuzziness.fromEdits(this.editDistance)).boost(boostFactor) :
                QueryBuilders.queryStringQuery(trimmedStr)
    ...
    public void setTAddresses(String tAddresses) {
        this.tAddresses = Arrays.asList(tAddresses.split(","));
    }

    public void setEditDistance(int editDistance) {
        Preconditions.checkArgument(editDistance <= Fuzziness.TWO.asInt());
        this.editDistance = editDistance;
    }

    public static class Builder {

        private String tAddresses;
        ...
        public Builder setTAddresses(String tAddresses) {
        ...
        public Builder setMaxSearchHits(int maxSearchHits) {
            this.maxSearchHits = maxSearchHits;
            return this;
        }
        ...
        public Builder setEditDistance(int editDistance) {
            this.editDistance = editDistance;
            return this;
        }

        public Builder setLogESQueries(boolean logESQueries) {
            this.logESQueries = logESQueries;
            return this;
        }

        public ElasticSearchDatabase build() {
            final ElasticSearchDatabase elasticSearchDatabase = new ElasticSearchDatabase();
            if (this.tAddresses != null) {
                elasticSearchDatabase.setTAddresses(this.tAddresses);
            }
            if (this.indexName != null) {
                elasticSearchDatabase.setIndexName(indexName);
            }
            if (this.maxSearchHits > 0) {
                elasticSearchDatabase.setMaxSearchHits(this.maxSearchHits);
            }
            if (this.cluster != null) {
                elasticSearchDatabase.setCluster(this.cluster);
            }
            elasticSearchDatabase.setOverwrite(this.overwrite);
            elasticSearchDatabase.setEditDistance(this.editDistance);
            elasticSearchDatabase.logESQueries = this.logESQueries;
